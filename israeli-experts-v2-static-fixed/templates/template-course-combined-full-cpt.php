<?php
/**
 * Template Name: תבנית עמוד קורס משולב - טקסט מתוקן
...
function pablo_course_combined_meta_text(int $post_id, string $key, string $fallback = ''): string
{
    $value = get_post_meta($post_id, $key, true);
    return is_string($value) && trim($value) !== '' ? $value : $fallback;
}

function pablo_course_combined_meta_html(int $post_id, string $key, string $fallback = ''): string
{
    $value = get_post_meta($post_id, $key, true);
    return is_string($value) && trim($value) !== '' ? $value : $fallback;
}

function pablo_course_combined_get_placeholder_image(): string
{
...
function pablo_course_combined_get_lecturer_image_url(int $lecturer_id): string
{
    ...
    return pablo_course_combined_get_placeholder_image();
}

function pablo_course_combined_get_lecturer_doc_url(int $lecturer_id): string
{
...
function pablo_course_combined_get_lecturer_viewer_url(int $lecturer_id): string
{
    $doc_url = pablo_course_combined_get_lecturer_doc_url($lecturer_id);
    if ($doc_url === '') {
        return '#';
    }
...
$register_url   = pablo_course_combined_meta_text($post_id, 'pablo_register_url', home_url('/טופס-הרשמה-לקורס-משולב/'));
$register_label = pablo_course_combined_meta_text($post_id, 'pablo_register_label', 'להרשמה לקורס לחץ כאן');

$features_html = pablo_course_combined_meta_html(
    $post_id,
    'pablo_combined_features_html',
    '<p><span>אפשר להזין כאן מאפייני קורס מותאמים דרך meta key: pablo_combined_features_html</span></p>'
);

$syllabus_html = pablo_course_combined_meta_html(
    $post_id,
    'pablo_combined_syllabus_html',
    '<div class="pablo-course-empty">לא הוזן עדיין סילבוס לעמוד זה. אפשר להדביק את כל ה-HTML הישן לשדה המטא pablo_combined_syllabus_html.</div>'
);

$lecturers_query = new WP_Query([
    'post_type'      => 'pbl_lect103',
    'post_status'    => 'publish',
    'posts_per_page' => -1,
    'orderby'        => [
        'menu_order' => 'ASC',
...
<div class="pablo-course-page">
    ...
    <ul class="pablo-course-tabs" role="tablist">
        <li><button type="button" class="pablo-course-tab-btn" data-tab-target="pablo-course-tab-description-103"><span class="dashicons dashicons-book-alt"></span> תיאור</button></li>
        <li><button type="button" class="pablo-course-tab-btn" data-tab-target="pablo-course-tab-syllabus-103"><span class="dashicons dashicons-archive"></span> סילבוס</button></li>
        <li><button type="button" class="pablo-course-tab-btn is-active" data-tab-target="pablo-course-tab-lecturers-103"><span class="dashicons dashicons-businessperson"></span> מרצים</button></li>
    </ul>

    <section id="pablo-course-tab-description-103" class="pablo-course-panel">
    ...
    <section id="pablo-course-tab-syllabus-103" class="pablo-course-panel">
        <div class="pablo-course-syllabus-wrap">
            <?php echo wp_kses_post($syllabus_html); ?>
        </div>
    </section>

    <section id="pablo-course-tab-lecturers-103" class="pablo-course-panel is-active">
        <div class="pablo-course-lecturers-wrap">
            <?php if ($lecturers_query->have_posts()) : ?>
                <?php while ($lecturers_query->have_posts()) : $lecturers_query->the_post(); ?>
                    ...
                    $viewer_url     = pablo_course_combined_get_lecturer_viewer_url($lecturer_id);
                    $card_image_url = pablo_course_combined_get_lecturer_image_url($lecturer_id);
                    ?>
